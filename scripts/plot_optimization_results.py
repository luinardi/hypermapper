...
def main(
    ...
    output_metric = param_space.get_optimization_parameters()[0] # only works for mono-objective
    doe_size = config['design_of_experiment']['number_of_samples']
    feasibility_flag = param_space.get_feasible_parameter()[0] # returns a list, we just want the name
    ...
    if expert_configuration is not None:
        ...
        plt.plot(list(range(max_iters)), expert_data, color="black", linewidth=linewidth, linestyle="solid")
    ...
    for key_idx, key in enumerate(regrets.keys()):
        ...
        legend_elements.append(Line2D([0], [0], color=next_color, label=label, linewidth=linewidth))

    if expert_configuration is not None:
        legend_elements.append(Line2D([0], [0], color="black", linewidth=linewidth, linestyle="solid", label="Expert Configuration"))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Plot regret for multiple optimization runs.')
    parser.add_argument('--configuration_file', '-j', dest='config', action='store', default=None)
    parser.add_argument('--data_dirs', '-i', dest='data_dirs', action='store', default=None, nargs='+')
    parser.add_argument('--labels', '-l', dest="labels", action="store", default=None, nargs="+")
    parser.add_argument('--minimum', '-min', dest='minimum', action='store', default=0, type=int)
    parser.add_argument('--output_dir', '-od', dest='out_dir', action="store", default="")
    parser.add_argument('--budget', '-b', dest='budget', action="store", type=int, default=None)
    parser.add_argument('--output_file', '-o', dest='outfile', action='store', default=None)
    ...
    parser.add_argument('--x_label', '-xl', dest='x_label', action='store', default=None)
    parser.add_argument('--y_label', '-yl', dest='y_label', action='store', default=None)
    parser.add_argument('--ncol', '-nc', dest='ncol', action="store", type=int, default=4)
    parser.add_argument('--expert_configuration', '-exp', dest='expert_configuration', action="store", type=float, default=None)
