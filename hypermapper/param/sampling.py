    ...
    if allow_repetitions or param_space.has_real_parameters:
        ...
        if param_space.size <= n_total_samples:
                        ...
                        np.exp(
                            np.sum(
                                [
                                    np.log(param.pdf(x))
                                    for x, param in zip(
                                        remaining_space[configuration_idx],
            ...
            chosen_indices = np.random.choice(
                remaining_space_size, n_samples, replace=False, p=probabilities
            )
            ...
            for trial in range(30):
                ...
                for i, parameter in enumerate(parameters):
                    if sampling_method == "using_priors":
                        ...
                        new_configurations[:, i] = parameter.sample(
                            size=n_samples, uniform=True
                        )

                new_configurations = param_space.filter_out_previously_run(
                    new_configurations, previously_run
                )
                for c in new_configurations:
                    previously_run[
                        param_space.get_unique_hash_string_from_values(c)
                    ] = -1
...
def _random_sample_constrained(
    ...
    if allow_repetitions or param_space.has_real_parameters:
        tree_samples = param_space.chain_of_trees.sample(
            n_samples,
            sampling_method,
            [p.name for p in param_space.cot_parameters],
            allow_repetitions=True,
        )
        non_tree_samples = _non_tree_constrained_sample(
            param_space, param_space.non_cot_parameters, n_samples, sampling_method
        )
        ...
        samples = torch.cat((tree_samples, non_tree_samples), dim=1)[
            :, param_space.cot_remap
        ]
        return samples
    else:
        if param_space.chain_of_trees.get_size() <= n_total_samples:
            ...
            if not previously_run_configurations:
                ...
                return param_space.filter_out_previously_run(
                    too_many_samples, previously_run_configurations
                )[:n_samples]
...
def _non_tree_constrained_sample(
    ...
    for trial in range(50):
        ...
        transformed_configurations = {
            ...
            for parameter, parameter_value in zip(
                ...
                [
                    ...
                    for i in zip(
                        *param_space.convert(
                            trial_samples, "internal", "original", parameters
                        )
                    )
                ],
            )
        ...
        feasible = constraints.evaluate_constraints(
            param_space.non_cot_constraints, transformed_configurations
        )
...
def get_random_configurations(
    param_space: Space, use_priors=True, n_samples=1
) -> torch.Tensor:
    """
    Input:
        - param_space: Space object
