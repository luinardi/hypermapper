...
def local_search(
    ...
    if number_of_cpus == 1:
        ...
        # define function to run in parallel (nested to avoid more argument passing and easier to follow
        def parallel_optimization_function(
            optimization_function_parameters, input_queue, output_queue, proc
        ):
        ...
        processes = [
            Process(
                ...
                args=(optimization_function_parameters, input_queue, output_queue, i),
            )
    ...
    def sequential_local_search(
        ...
        sys.stdout.write_to_logfile("Running sequential_local_search \n")
        while len(input_list) > 0:
            config = input_list.pop(0)
            if config is None:
                break
            ...
            while configuration is not None:
                ...
                if are_configurations_equal(best_neighbor, configuration, input_params):
                    ...
                    output_list.append(
                        {"data_array": iteration_data_array}
                    )
                    configuration = None
                else:
                    sys.stdout.write_to_logfile((
    ...
    if number_of_cpus == 1:
        input_list = []
        output_list = []
        # puts each configuration in a list to be evaluated in sequence
