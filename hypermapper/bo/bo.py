...
from hypermapper.util.util import (
    sample_weight_flat, update_mean_std,
)
...
def main(settings, black_box_function=None):
    ...
    if "feasible_output" in settings:
        enable_feasible_predictor = settings["feasible_output"]["enable_feasible_predictor"]
    else:
        enable_feasible_predictor = False

    data_array = DataArray(torch.Tensor(), torch.Tensor(), torch.Tensor(), torch.Tensor(), torch.Tensor())
    ...
    if absolute_configuration_index < settings["design_of_experiment"]["number_of_samples"]:
        ...
        doe_data_array = param_space.run_configurations(doe_parameter_array, beginning_of_time, settings, black_box_function)
    ...
    if enable_feasible_predictor:
        while (True not in data_array.feasible_array) and settings["optimization_iterations"] >= iteration_number:
            print("Warning: all points are invalid, random sampling more configurations.")
            ...
            tmp_data_array = param_space.run_configurations(tmp_parameter_array, beginning_of_time, settings, black_box_function)
    ...
    objective_means, objective_stds = update_mean_std(feasible_data_array.metrics_array, settings)
    ...
    if settings["time_budget"] > 0:
        print("starting optimization phase, limited to run for ", settings["time_budget"], " minutes")
    ...
    for iteration in range(iteration_number, settings["optimization_iterations"] + 1):
        print("Starting optimization iteration:", iteration, "sample no:", data_array.len)
        ...
        if random.uniform(0, 1) > settings["epsilon_greedy_threshold"]:
            ...
            if regression_models is None:
                ...
                objective_weights = sample_weight_flat(settings["optimization_objectives"])[0]
        ...
        new_data_array = param_space.run_configurations(best_configuration.unsqueeze(0), beginning_of_time, settings, black_box_function)
